var validator = require("validator")
  ...
  , async = require("async")
  , utils = require("./utils")
  , lang = 'es' // "es" , "fr" ," zh" and the rest of them
  , mess = require("../languages/"+ lang); 
...
Field.prototype.arrLength = function (from, to) {
  return this.add(function (arr) {
    if (value.length < from) {
      return { error: message || e.message || mess.arrLengthShort };
    }
    if (value.length > to) {
      return { error: message || e.message || mess.arrLengthLong };
    }
    return { valid: true };
  });
}
...
Field.prototype.custom = function(func, message) {
  ...
  if (func.length == 3) {
    return this.add(function(value, source, cb) {
      func(value, source, function(err, result) {
        if (err) return cb(new Error(message || err.message || mess.custom));
  ...
  return this.add(function (value, source) {
    ...
    try {
      ...
      return { error: message || e.message || mess.custom };
    }
    // Functions that return values are filters.
...
var MESSAGES = {
    isDate:           mess.isDate
  , isEmail:          mess.isEmail
  , isUrl:            mess.isUrl
  , isIP:             mess.isIP
  , isAlpha:          mess.isAlpha
  , isAlphanumeric:   mess.isAlphanumeric
  , isNumeric:        mess.isNumeric
  , isLowercase:      mess.isLowercase
  , isUppercase:      mess.isUppercase
  , isInt:            mess.isInt
  , notEmpty:         mess.notEmpty
};
console.log(MESSAGES.isDate)
...
Field.prototype.contains = function (test, message) {
  return this.add(function(value) {
    try {
      ...
      return { error: message || mess.contains };
    }
    return { valid: true };
  });
};
...
Field.prototype.notContains = function (test, message) {
  return this.add(function (value) {
    try {
      ...
      return { error: message || mess.notContains };
    }
    return { valid: true };
  });
};
...
Field.prototype.equals = function (other, message) {
  ...
  return this.add(function (value, source) {
    ...
    if (value != test) {
      return { error: message || mess.equals + String(test) };
    }
    return { valid: true };
  });
};
...
Field.prototype.isNumeric = function (message) {
  return this.add(function (value) {
    if (object.isNumber(value) || (object.isString(value) && value.match(/^[-+]?[0-9]*\.?[0-9]+$/))) {
      ...
      return { error: message || mess.isNumeric };
    }
    return { valid: true };
  });
};
...
Field.prototype.isFloat = Field.prototype.isDecimal = function (message) {
  return this.add(function (value) {
    if ((object.isNumber(value) && value % 1 == 0) || (object.isString(value) && value.match(/^[-+]?[0-9]*\.[0-9]+$/))) {
      ...
      return { error: message || mess.isFloat };
    }
    return { valid: true };
  });
};
...
Field.prototype.regex = Field.prototype.is = function (pattern, modifiers, message) {
  ...
  return this.add(function (value) {
    if (pattern.test(value) === false) {
      return { error: message || mess.regex };
    }
    return { valid: true };
  });
};
...
Field.prototype.notRegex = Field.prototype.not = function(pattern, modifiers, message) {
  ...
  return this.add(function(value) {
    if (pattern.test(value) === true) {
      return { error: message || mess.notRegex };
    }
    return { valid: true };
  });
};
...
Field.prototype.required = function (placeholderValue, message) {
  ...
  return this.add(function (value) {
    if (!utils.hasValue(value) || value == placeholderValue) {
      return { error: message || mess.required };
    }
    return { valid: true };
  });
};
...
Field.prototype.minLength = function (length, message) {
  return this.add(function(value) {
    if (value.toString().length < length) {
      return { error: message || mess.minLength };
    }
    return { valid: true };
  });
};
...
Field.prototype.maxLength = function (length, message) {
  return this.add(function(value) {
    if (value.toString().length > length) {    
      return { error: message || mess.maxLength };
    }
    return { valid: true };
  });
};
...
Field.prototype.customValidator = function(func, message) {
  return this.add(function(value, source) {
    try {
      ...
      return { error: message || e.message || mess.customValidator};
    }
    return { valid: true };
  });
};
